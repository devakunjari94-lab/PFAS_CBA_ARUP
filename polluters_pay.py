...
# 🔐 PASSWORD PROTECTION
# ======================
APP_PASSWORD = st.secrets["PASSWORD"]  # from Streamlit Secrets

# Initialize session state for authentication
if "authenticated" not in st.session_state:
    st.session_state.authenticated = False

# Password input (shown only if not authenticated)
if not st.session_state.authenticated:
    password_input = st.text_input("Enter password to access the PFAS Polluter-Pays Calculator", type="password")
    if password_input:
